...
"""
SYNOPSIS
    Merge data from multiple input MAF files into a single output MAF file.

NOTES

    (a) Retain only the 34 standard MAF columns from the input MAF files.
        All other columns in the input MAF file are discarded.
...
EXAMPLES
    ...
    # Merge 3 input MAF files (two from DFCI and one from METABRIC).
    ./merge_maf.py \
        --in_mafs ../examples/inputs/private/dfci.DFCI-ONCOPANEL-1.r4.maf \
                  ../examples/inputs/private/dfci.DFCI-ONCOPANEL-2.r4.maf \
                  ../examples/inputs/public/metabric/metabric.maf \
        --out_maf private/merged.maf
...
EDGE_CASES
    ...
    ./merge_maf.py \
        --in_mafs ../examples/inputs/private/dfci.DFCI-ONCOPANEL-1.r4.maf \
                  ../examples/inputs/private/dfci.DFCI-ONCOPANEL-1.r4.maf \
        --out_maf private/tmp.merged.maf
    ...
    ./merge_maf.py \
        --in_mafs ../examples/inputs/private/dfci.DFCI-ONCOPANEL-1.r4.maf \
                  ../examples/inputs/private/dupl.dfci.DFCI-ONCOPANEL-1.r4.maf \
        --out_maf private/tmp.merged.maf
...
std_MAF_columns = ["Hugo_Symbol", "Entrez_Gene_Id", "Center", "NCBI_Build",
...
def main(options):
    ...
    for in_maf_file in options.in_mafs:
        ...
        # Reorder and retain only the standard MAF columns + 'Source_Maf'
        in_maf_df = in_maf_df[std_MAF_columns + ['Source_Maf']]
...
def check_column_header(maf_df, in_maf_file):
    ...
    missing_columns = list(set(std_MAF_columns) - set(columns))

    if len(missing_columns) > 0:
        raise Exception("Missing standard MAF column(s) %s in input_maf %s." % (missing_columns,
                                                                                repr(in_maf_file)))

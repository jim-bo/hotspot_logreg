...
"""
SYNOPSIS
    Prepare the ICGC BRCA-KR donor and sample files into a clinical tsv format that is suitable for input into
    the Hotspots 'Mutation_Counts' pipeline.

NOTES
    - No hormone receptor status information
    - No histology type information
    - The only values for the "donor_relapse_type" column are 'local recurrence and distant metastasis', and
       'distant recurrence/metastasis', so I am assigning "Metastatic" as the
       Biopsy_Site_Type for all samples.

    - Data was downloaded from https://dcc.icgc.org/releases/current/Projects/BRCA-KR.

EXAMPLES
    ```
    ./prep_icgc_brcaKR_clinical.py \
        --donor-file ./donor.BRCA-KR.tsv \
        --sample-file ./sample.BRCA-KR.tsv \
        --out-clinical ./icgc_brca_kr.clinical.r1.tsv
    ```
